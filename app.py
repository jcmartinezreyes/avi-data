...
# ---------------------------------------------------------
# 3. ALOJAR LOTE CONDICIONAL POR PROPÓSITO Y SEXADO
# ---------------------------------------------------------
    ...
    if res_galpones.data:
        ...
        with col_p2:
            # Lógica condicional de opciones según el propósito
        ...
        # Captura de cantidad de aves según la modalidad seleccionada
        hembras = 0
        machos = 0
        ...
        if modalidad_sexo == "Sexado (Hembras / Machos)":
            ...
            with col_m:
                machos = st.number_input("Aves Machos Iniciales", min_value=0, value=1000, step=100)
        elif modalidad_sexo == "Mixto (As Sexed)":
            total_mixto = st.number_input("Cantidad de Aves Mixtas (Sin sexar)", min_value=0, value=20000, step=500)
            # Para fines estadísticos en lotes mixtos, se toma el 50% equivalente de cada sexo
            hembras = total_mixto // 2
            machos = total_mixto - hembras
        elif modalidad_sexo == "Solo Hembras":
            hembras = st.number_input("Cantidad de Aves Hembras", min_value=0, value=15000, step=500)
            machos = 0
            
        totales = hembras + machos
        st.info(f"📊 **Total de Aves Alojadas en este Galpón:** {totales:,} (Hembras: {hembras:,} | Machos: {machos:,})")
        ...
        if st.button("Alojar Lote") and codigo_lote:
            ...
            # Intentar insertar incluyendo el campo 'proposito'
            try:
                supabase.table("lotes").insert(datos_lote).execute()
            except Exception:
                # Fallback si la columna propósito aún no existe en Supabase
                datos_lote.pop("proposito", None)
                supabase.table("lotes").insert(datos_lote).execute()
                
            st.success(f"¡Lote '{codigo_lote}' alojado correctamente con propósito {proposito}!")
    else:
